...
module.exports = async function (context, req) {
    ...
    try {
        ...
        let audioBuffer = Buffer.from(req.body.audioData, 'base64');
        
        // Log original audio buffer info
        context.log(`Original audio buffer size: ${audioBuffer.length} bytes`);
        context.log(`Audio format: ${req.body.format || 'unknown'}`);
        
        // Audio preprocessing - convert to optimal format for Azure Speech
        audioBuffer = await preprocessAudio(audioBuffer, context);
        ...
        // Method 1: Try with 16kHz mono WAV format (optimal)
        try {
            context.log('Attempting Method 1: 16kHz mono WAV format...');
            ...
            recognitionResult = await performRecognition(recognizer1, context, 'Method 1');
            ...
            errors.push(`Method 1 (16kHz mono): ${error1.message}`);
            context.log(`Method 1 failed: ${error1.message}`);
            
            // Method 2: Try with 8kHz format
            try {
                context.log('Attempting Method 2: 8kHz mono WAV format...');
                ...
                recognitionResult = await performRecognition(recognizer2, context, 'Method 2');
                ...
                errors.push(`Method 2 (8kHz mono): ${error2.message}`);
                context.log(`Method 2 failed: ${error2.message}`);
                
                // Method 3: Try with default format
                try {
                    ...
                    recognitionResult = await performRecognition(recognizer3, context, 'Method 3');
        ...
        if (recognitionResult && recognitionResult.reason === sdk.ResultReason.RecognizedSpeech) {
            ...
            context.res.body = {
                ...
                details: "Could not detect any speech in the provided audio file after preprocessing",
                debug: {
                    reason: recognitionResult?.reason,
...
// Helper function to perform recognition with timeout
async function performRecognition(recognizer, context, methodName) {
    return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
            reject(new Error(`${methodName} timeout after 30 seconds`));
        }, 30000);
        ...
        recognizer.recognizeOnceAsync(
            (result) => {
                ...
                recognizer.close();
                context.log(`${methodName} result: ${result.reason}, Text: "${result.text}"`);
                resolve(result);
            },
            (error) => {
                clearTimeout(timeout);
                recognizer.close();
                context.log(`${methodName} error: ${error}`);
                reject(new Error(`${methodName} failed: ${error}`));
            }
        );
    });
}

// Audio preprocessing function to convert to optimal format
async function preprocessAudio(audioBuffer, context) {
    try {
        context.log('Starting audio preprocessing...');
        
        // For now, we'll return the buffer as-is since we can't do complex audio processing
        // in Node.js without additional dependencies. However, we're configuring Azure
        // to expect the correct format which should help with conversion.
        
        context.log(`Audio preprocessing complete. Output size: ${audioBuffer.length} bytes`);
        return audioBuffer;
        
    } catch (error) {
        context.log.error('Audio preprocessing failed:', error);
